...
junshisan <- zaimu$純資産合計
fusai <- zaimu$負債合計
koteishisan <- zaimu$固定資産合計
koteifusai <- zaimu$固定負債合計
ryuudoushisan <- zaimu$流動資産合計
ryuudoufusai <- zaimu$流動負債合計
touzashisan <- zaimu$当座資産合計
#自己資本比率 = 純資産/(負債+純資産)
...
#損益計算書による収益力の分析
uriagesourieki <- zaimu$売上総利益
uriagedaka <- zaimu$売上総利益
eigyourieki <- zaimu$営業利益
keijourieki <- zaimu$経常利益
toukijunrieki <- zaimu$当期利益
#売上総利益率 = 売上総利益/売上高
...
tanaoroshishisan <- zaimu$棚卸資産合計
uriagesaiken <- zaimu$受取手形.売掛金
yuukeikoteishisan <- zaimu$有形固定資産合計
#棚卸資産回転率 = 売上高 / 棚卸資産
tanaoroshishisankaitenritsu <- uriagedaka / tanaoroshishisan
#売上債権回転率 = 売上高 / 売上債権
uriagesaikenkaitenritsh <- uriagedaka / uriagesaiken
...
#企業力指数による伝統的な分析手法の総合
jikoshihon <- zaimu$純資産合計 + zaimu$資本金 + zaimu$資本準備金
shisan <- zaimu$資産合計
#収益力指数 = 売上高/(売上高-経常利益)
...
#付加価値分析
jinkenhi <- zaimu$X.販.人件費.福利厚生費 + zaimu$X.製.労務費.福利厚生費
chintairyou <- zaimu$X.販.賃借料 + zaimu$X.製..うち賃借料.
sozeikouka <- zaimu$X.販.租税公課 + zaimu$X.製..うち租税公課.
genkasyoukyakuhi <- zaimu$減価償却実施額
juugyouinsuu <- zaimu$従業員数.単位.人.
yuukeikoteishisan <- zaimu$有形固定資産合計
#付加価値(控除法) = 売上高 - 前給付費用
...
zaimu <- read.csv("~/Desktop/R/zaimu.csv", fileEncoding="CP932", header=TRUE, quote="\"")
View(zaimu)
#zaimu ：データベース
...
keiei <- function(zaimu,meigara,year){
  ...
  for(i in 1:n){
    ...
    uriagesaiken <- data$X.ＩＦＲＳ..うち受取手形.売掛金. + X.受取手形.
  ...
  result <- list(taishaku,soneki,kouritsu,kigyouryoku)
...
year <- c(1985:2008)
keiei(zaimu,8267,year)

#a <- as.vector(soneki[2,])
#plot(soneki[2,])
